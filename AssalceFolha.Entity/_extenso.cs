...
namespace AssalceFolha.Entity
{
    ...
    public static class _extenso
    {
        ...
        private static string ConversaoRecursiva(Int64 intNumero)
        {
            ...
            if ((intNumero >= 1) && (intNumero <= 19)) return strUnidades[intNumero];
        ...
        public static string NumeroParaExtenso(double decNumero)
        {
            return NumeroParaExtenso(Convert.ToDecimal(decNumero));
        }

        public static string NumeroParaExtenso(int decNumero, bool moeda = true)
        {
            return NumeroParaExtenso(Convert.ToDecimal(decNumero), moeda);
        }

        public static string NumeroParaExtenso(decimal decNumero, bool moeda = true)
        {
            string strRetorno = "";
            if ((decNumero >= decMin) && (decNumero <= decMax))
            {
                ...
                if (moeda) strRetorno  += (string)(intInteiro <= 1 ? strMoeda : strMoedas);

                if (intCentavos > 0 & moeda)
                {
