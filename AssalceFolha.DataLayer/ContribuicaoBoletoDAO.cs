...
{
    public class ContribuicaoBoletoDAO : EntidadeDAO<ContribuicaoBoleto>
    {
        public ContribuicaoBoleto SelecionarPorMatricula(int _matricula)
        {
            try
            {
                string _sql = " SELECT * FROM TB_CONTRIBUICAO_BOLETO WHERE NR_MATRICULA = " + _matricula.ToString();
        ...
        public void ExcluirAlone(ContribuicaoBoleto _entidade)
        {
            try
            {
                using (MySqlConnection con = new MySqlConnection(ConfigurationManager.ConnectionStrings["STRConexao"].ConnectionString))
                {
                    ...
                    try
                    {
                        ContribuicaoBoleto entidade = _entidade;
