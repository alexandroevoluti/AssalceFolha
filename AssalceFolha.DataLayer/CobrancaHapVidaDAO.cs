...
{
    public class CobrancaHapVidaDAO : EntidadeDAO<CobrancaHapVida>
    {
        public List<CobrancaHapVida> Listar(int _competencia)
        {
            try
            {
                string _sql = " SELECT * FROM TB_COBRANCA_HAPVIDA WHERE NR_COMPETENCIA = " + _competencia.ToString();
        ...
        public void AtualizarCPF(string _cpfAtual, string _nome, string _novoCPF)
        {
            try
            {
                string _sql = " UPDATE TB_COBRANCA_HAPVIDA  SET CPF = '{0}' WHERE CPF = '{1}'";
        ...
        public List<CobrancaHapVida> Listar(string _nome, string _ano)
        {
            try
            {
                ...
                string _sql = " SELECT * FROM TB_COBRANCA_HAPVIDA ";
        ...
        public List<ValoresCobrancaHapVidaDTO> PesquisarValores(Associado _associao, int _ano)
        {
            try
            {
                ...
                List<ValoresCobrancaHapVidaDTO> _lista = null;
                ...
                {
                    _lista = new List<ValoresCobrancaHapVidaDTO>();

                    foreach (DataRow _row in _ds.Tables[0].Rows)
                    {
                        _lista.Add(new ValoresCobrancaHapVidaDTO()
                        {
        ...
        public List<CobrancaHapVida> Incluir(List<CobrancaHapVida> _cobrancas, MySqlConnection con, MySqlTransaction tran)
        {
            try
            {
                List<CobrancaHapVida> _retorno = new List<CobrancaHapVida>();

                foreach (CobrancaHapVida item in _cobrancas)
                {
